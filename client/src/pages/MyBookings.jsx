          ...
          >
            ...
            <div className="flex flex-col items-start justify-center pt-3">
                <div className="flex items-center gap-2">
                    ...
                    <p className={`text-sm ${item.isPaid ? 'text-green-500' : 'text-red-500'}`}>{item.isPaid ? 'Paid' : 'Unpaid'}</p>
                </div>
                {!item.isPaid && (<button className="px-4 py-1.5 mt-4 text-xs border border-gray-400 rounded-full hover:bg-gray-50 transition-all cursor-pointer">Pay Now</button>)}
            </div>
          </div>
        ))}
      </div>
